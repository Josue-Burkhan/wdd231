...
function renderCourses(filter) {
  ...
  courseButtonsDiv.innerHTML = filteredCourses.map(course => `
      <button 
          style="background-color: ${course.completed ? '#1D2C40' : 'transparent'}; 
                 color: ${course.completed ? '#fff' : '#000'};">
...
categoriesDiv.innerHTML = subjects.map(subject => `<button>${subject}</button>`).join('');
